-- Deploy disputeworkflow:create_camunda_engine to pg
...
create index if not exists ACT_IDX_EXE_ROOT_PI on ACT_RU_EXECUTION (ROOT_PROC_INST_ID_);
...
create index if not exists ACT_IDX_JOB_EXCEPTION on ACT_RU_JOB (EXCEPTION_STACK_ID_);
