...
private const val BUYER_ID_PROPERTY = "buyer_id"
private const val DISPUTE_ACTIVE_USER = "active_user"

class SetBuyerAsActiveUserDrcListener(
    ...
    override fun doExecute(execution: DelegateExecution) {
        ...
        val buyerId = execution.getLongProcessVariableOrNull(BUYER_ID_PROPERTY)
        if (buyerId == null) {
            logger.error { "Unable to find buyer ID in the task context." }
            throw BpmnError(
                TaskErrorCode.FAILURE.code,
                "$BUYER_ID_PROPERTY must not be null or empty."
            )
        }

        drcClient.updateDispute(
            disputeId,
            UpdateActiveUserIdDisputeRequest(buyerId)
        ).block()

        execution.setVariable(DISPUTE_ACTIVE_USER, buyerId.toString())
    }
}